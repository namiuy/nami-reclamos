...
export function EmpresasList() {
  ...
  const [rows, set_rows] = useState<EmpresaRow[]>([])
  ...
  const [selected_empresa_id, set_selected_empresa_id] = useState<number | null>(null)
  ...
  const fetch_empresas = async () => {
    set_loading(true)
    try {
      const response = await fetch(
        `/api/empresas?page=${pagination_model.page + 1}&page_size=${pagination_model.pageSize}`
      )
      ...
      if (response.ok) {
        ...
        set_snackbar({
          open: true,
          message: data.error || 'Error al cargar empresas',
          severity: 'error',
        })
      }
    } catch (error) {
  ...
  const handle_edit = (id: number) => {
    router.push(`/empresas/${id}/editar`)
  }

  const handle_delete_click = (id: number) => {
    set_selected_empresa_id(id)
    set_delete_dialog_open(true)
  }

  const handle_delete_confirm = async () => {
    if (!selected_empresa_id) return

    try {
      const response = await fetch(`/api/empresas/${selected_empresa_id}`, {
        method: 'DELETE',
      })
  ...
  const handle_new_empresa = () => {
    router.push('/empresas/nuevo')
  }
    ...
    },
    {
      ...
      getActions: (params) => [
        <GridActionsCellItem
          ...
          icon={<EditIcon />}
          label="Editar"
          onClick={() => handle_edit(params.row.empresa_id)}
        />,
        <GridActionsCellItem
          key="delete"
          icon={<DeleteIcon />}
          label="Eliminar"
          onClick={() => handle_delete_click(params.row.empresa_id)}
        />,
      ],
    },
  ]
